...
from core_apps.stream_v4.serializers import VideoMetaDataDRMV4Serializer
from core_apps.stream_v4.utils import process_and_save_video_local_drm_v4
from core_apps.stream_v4.paginations import VideoMetaDataDRMV4PageNumberPagination

# Celery Tasks
from core_apps.stream_v4.tasks import (
...
class UploadVideoVoDDRMV4API(APIView):
    ...
    def post(self, request, format=None):
        ...
        result = process_and_save_video_local_drm_v4(request=request)
        ...
        try:
            ...
            serializer = VideoMetaDataDRMV4Serializer(data=db_data)
...
class StreamVideoVoDDRMV4API(APIView):
    ...
    def get(self, request, video_id):
        ...
        try:
            video_metadata = VideoMetaDataDRMV4.objects.get(id=video_id)
            serializer = VideoMetaDataDRMV4Serializer(video_metadata)
            return Response(
            ...
            Drf_ValidationError,
            VideoMetaDataDRMV4.DoesNotExist,
        ) as err:
            return Response(
                {
                    "data": {
...
class AllVideosAPIDRMV4(APIView):
    """A simple API for all available video of cutetube. Paginated response."""

    permission_classes = [AllowAny]
    pagination_class = VideoMetaDataDRMV4PageNumberPagination

    def get(self, request):
        """Get Video Metadata Paginated Response of size 10"""

        video_metadatas = VideoMetaDataDRMV4.objects.all()

        paginator = self.pagination_class()
        paginated_videos = paginator.paginate_queryset(video_metadatas, request)

        serializer = VideoMetaDataDRMV4Serializer(paginated_videos, many=True)
